...
function baloon(inf,sty){
	if(!document.tooltip){
		document.tooltip=document.createElement("div");
		document.body.appendChild(document.tooltip);
		document.tooltip.className="tooltip";
		document.tooltip.innerHTML="<div></div>";
		createListener("mouseout",function(){baloon()},document);
		createListener("mousedown",function(){baloon()},document);
	}
	document.tooltip.className=(sty)?"tooltip "+sty:"tooltip";
	if(inf){
		document.tooltip.firstChild.innerHTML=inf;
		if(document.tooltip.offsetWidth>170) document.tooltip.style.width="170px";
		document.tooltip.hlim=(_ie)?document.body.offsetWidth-document.tooltip.offsetWidth:window.innerWidth-document.tooltip.offsetWidth;
		document.tooltip.vlim=(_ie)?document.body.offsetHeight-document.tooltip.offsetHeight:window.innerHeight-document.tooltip.offsetHeight;
		createListener("mousemove",dragBaloon,document);
	}else{
		document.tooltip.style.width="";
		document.tooltip.firstChild.innerHTML="";
		deleteListener("mousemove",dragBaloon,document);
		document.tooltip.style.left="-1000px";	
	}
}

function dragBaloon(e){
	var tip=document.tooltip;
	if(!e){e=event}
	//debug(e.clientX,e.clientY,document.tooltip.hlim,document.tooltip.vlim)
	//tip.style.left=(e.clientX>=tip.hlim)?(e.clientX-document.tooltip.offsetWidth+8)+"px":e.clientX+"px";//-tip.offsetWidth/2; //LEFT
	tip.style.left=e.clientX-document.tooltip.offsetWidth/2+7+"px"// CENETR
	var _toppos=e.clientY-document.tooltip.offsetHeight-5;
	tip.style.top=(e.clientY+20>=tip.vlim)?(e.clientY-document.tooltip.offsetHeight-5)+"px":(e.clientY+20)+"px"; //BELOW
...
var BASE64 = {
	...
	decode : function (input) {
		var isEncoded=(/^([A-Za-z0-9+/]{4})*([A-Za-z0-9+/]{4}|[A-Za-z0-9+/]{3}=|[A-Za-z0-9+/]{2}==)$/i.test(input));
		if(!isEncoded){
			return input;
		}
		var output = "";
